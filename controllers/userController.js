...
exports.sendLLSMSController = async (req, res) => {
  ...
  await User.findByIdAndUpdate(
    getUser._id,
    { targetId: `${req.params.num}-${newUUID}` },
    (err) => {
      if (err) {
        console.log(err);
      }
  ...
  var options = {
    ...
    message: `Submit your consent at ${process.env.HOST_URL}/giveConsent/${req.params.num}-${newUUID}`,
    numbers: [req.params.num],
  };
...
exports.postConsentController = async (res, req) => {
  ...
  const llUser = await User.findOne(
    { phoneNumber: 9906143871 },
    (err, data) => {
      //console.log(data);
      if (err) console.log(err);
    }
...
exports.getEkycController = catchAsync(async (res, req, next) => {
  ...
  try {
    await fs.writeFile(
      `./public/Ekyc/${req.req.body.fileName}`,
      req.req.body.ekyc,
      "base64",
      () => {
        return next();
